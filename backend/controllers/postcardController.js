...
export const createPostcard = async (req, res) => {
  const { title,date, message, sender_id, location, recipient_id } = req.body;
  const file = req.file;
  ...
  try {
    ...
    const { data, error: insertError } = await supabase
      .from('Postcard')
      .insert([{ title:title, description:message, sender_id:sender_id, image:image_url, model:location, reciever_id:recipient_id }])
      .select()
      .single();
